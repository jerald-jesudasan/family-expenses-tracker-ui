import { AlertCircle, Calendar, CheckCircle, Clock, Edit2, Link, Loader2, Pencil, Plus, Search, Trash2 } from 'lucide-react';
...
const categoryColors: Record<string, { bg: string; text: string; icon: string }> = {
  insurance:    { bg: 'bg-blue-100',    text: 'text-blue-700',    icon: 'shield' },
  education:    { bg: 'bg-purple-100',  text: 'text-purple-700',  icon: 'graduation-cap' },
  tax:          { bg: 'bg-amber-100',   text: 'text-amber-700',   icon: 'receipt' },
  maintenance:  { bg: 'bg-emerald-100', text: 'text-emerald-700', icon: 'wrench' },
  subscription: { bg: 'bg-pink-100',   text: 'text-pink-700',    icon: 'tv' },
  vehicle:      { bg: 'bg-orange-100',  text: 'text-orange-700',  icon: 'car' },
  other:        { bg: 'bg-gray-100',    text: 'text-gray-700',    icon: 'clipboard' },
};
              ...
              >
                ...
                <div className="flex justify-center gap-0.5 mt-1 sm:mt-2">
                  {monthPayments.slice(0, 4).map(sp => (
                    <span key={sp.id} className="inline-flex"><AppIcon name={sp.icon || 'calendar'} className="w-3 h-3 text-gray-500" /></span>
                  ))}
                  ...
                  <tr key={schedule.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 sm:px-6 py-4">
                      <span className="flex items-center gap-3">
                        <AppIcon name={schedule.icon || 'calendar'} className="w-5 h-5" />
                        <div>
                ...
                <div key={schedule.id} className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <span className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-xl bg-gray-100 text-gray-600">
                        <AppIcon name={schedule.icon || 'calendar'} className="w-5 h-5" />
                      </span>
...
function AddScheduleModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Payment Schedule" icon={<Calendar className="w-5 h-5" />} size="md">
...
function EditScheduleModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Payment Schedule" icon={<Pencil className="w-5 h-5" />} size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        <FormInput
          label="Schedule Name"
          value={formData.name}
...
function PayScheduleModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Record Payment" icon={<CheckCircle className="w-5 h-5" />} size="md">
      <div className="space-y-4">
        {/* Payment Details */}
        <div className="p-4 bg-gray-50 rounded-xl">
          <div className="flex items-center gap-3 mb-3">
            <span className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-xl bg-gray-100 text-gray-600">
              <AppIcon name={payment.icon || 'calendar'} className="w-5 h-5" />
            </span>
            <div>
