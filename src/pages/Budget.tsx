import { ArrowDownToLine, ArrowUpFromLine, Edit2, Info, Link as LinkIcon, Loader2, Plus, TrendingDown, TrendingUp, Trash2 } from 'lucide-react';
...
import { AppIcon, pickerIcons } from '../lib/icons';
import type { ExpenseCategory, IncomeSource } from '../services/budget';
...
export default function Budget() {
  ...
  const [formIcon, setFormIcon] = useState('banknote');
  ...
  const openEditModal = (item: IncomeSource | ExpenseCategory, type: 'income' | 'expense') => {
    ...
    setFormIcon(item.icon || 'banknote');
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div>
            ...
            <div className={`text-xl sm:text-2xl font-bold break-words ${surplus >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
              {surplus >= 0
                ? <TrendingUp className="inline w-5 h-5 mr-1 text-emerald-500" />
                : <TrendingDown className="inline w-5 h-5 mr-1 text-rose-500" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-emerald-50 to-white">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
              <h2 className="text-sm sm:text-base font-semibold text-gray-900 flex items-center gap-2"><ArrowDownToLine className="w-4 h-4 text-emerald-600" /> EXPECTED INCOME</h2>
              <button
                ...
                <div key={item.id} className="p-3 sm:p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0 w-full sm:w-auto">
                      <span className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-emerald-50 text-emerald-600">
                        <AppIcon name={item.icon || 'banknote'} className="w-4 h-4" />
                      </span>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-rose-50 to-white">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
              <h2 className="text-sm sm:text-base font-semibold text-gray-900 flex items-center gap-2"><ArrowUpFromLine className="w-4 h-4 text-rose-600" /> PLANNED EXPENSES</h2>
              <button
            ...
            {expenseCategories?.length === 0 ? (
              ...
              expenseCategories?.map((item) => (
                <div key={item.id} className={`p-3 sm:p-4 hover:bg-gray-50 transition-colors ${item.is_linked ? 'bg-blue-50/50' : ''}`}>
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0 w-full sm:w-auto">
                      <span className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-rose-50 text-rose-600">
                        <AppIcon name={item.icon || 'shopping-cart'} className="w-4 h-4" />
                      </span>
                      <div className="flex-1 min-w-0">
      ...
      <div className="text-xs sm:text-sm text-gray-500 flex items-start gap-2">
        ...
        <span>Auto-linked = Managed from Loans/Insurance module (cannot edit here)</span>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {pickerIcons.slice(0, 15).map((name) => (
                <button
                  key={name}
                  type="button"
                  onClick={() => setFormIcon(name)}
                  className={`w-10 h-10 rounded-lg border-2 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center ${formIcon === name ? 'border-indigo-500 bg-indigo-50 text-indigo-600' : 'border-gray-200 hover:border-gray-300 text-gray-500'}`}
                >
